...
> implements CacheMethods<R, L> {
  ...
  private isValidKey(key: CacheKey<R, L>): key is CacheKey<R, L> {
    return (
      this.ramKeys.includes(key as keyof R) ||
      this.localKeys.includes(key as keyof L)
    );
  }
   ...
   * @returns 默认值
   */
  private getDefaultValue<K extends CacheKey<R, L>>(
    key: K,
  ): CacheValue<R, L, K> {
   ...
   */
  async get<K extends CacheKey<R, L>>(
    key: K,
  ): Promise<CacheValue<R, L, K> | undefined> {
    if (!this.isValidKey(key)) {
      console.error(`Invalid cache key: ${String(key)}`);
      return undefined;
    }
   ...
   */
  async set<K extends CacheKey<R, L>>(
    key: K,
    value: CacheValue<R, L, K>,
  ): Promise<void> {
    if (!this.isValidKey(key)) {
      console.error(`Invalid cache key: ${String(key)}`);
      return;
